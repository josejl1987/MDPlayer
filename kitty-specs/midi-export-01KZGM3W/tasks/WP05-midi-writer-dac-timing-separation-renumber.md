...
create_intent:
- MDPlayer/tests/MDPlayer.Fmp.Tests/MidiFileWriterTests.cs
...
# Work Package Prompt: WP05 – MIDI Writer & DAC Timing Separation
...
5. DAC exporter no longer has a hidden origin-shift/negative-tick gap and performs no independent sample→tick conversion; DAC identity→note is independent of trigger→tick (§37, §38, test §68 — shared with WP04).
...
- `MidiEvent.cs` (owned by WP04 — referenced here, read-only): `MidiEventBase` (mutable `Tick`); records `MidiNoteEvent`, `Tempo`, `TimeSignature`, `MetaText`, `Marker`, `Program`, `Bank`, `PitchBend`, `BendRange`; `MidiEventOrder.Rank` deterministic ordering.
...
- Existing `MusicalMidiExporterTests` (owned by WP04) includes a writer-level test that constructs `MidiTrack`/`MidiNoteEvent` directly + `MidiFileWriter`, covering Format 1, conductor, determinism. New writer-level tests go in the new `MDPlayer/tests/MDPlayer.Fmp.Tests/MidiFileWriterTests.cs`.
...
- **Files**: `MDPlayer/src/MDPlayer.Fmp.Core/Timing/Midi/MidiFileWriter.cs`, `MDPlayer/tests/MDPlayer.Fmp.Tests/MidiFileWriterTests.cs`.
- **Parallel?**: Yes.
...
- **Files**: `MDPlayer/src/MDPlayer.Fmp.Core/Timing/Midi/MidiFileWriter.cs`, `MDPlayer/tests/MDPlayer.Fmp.Tests/MidiFileWriterTests.cs`.
- **Parallel?**: Yes.
- **Notes**: EOT emission stays writer-owned; WP04 asserts it at the exporter-output level.
...
- **Parallel?**: No — depends on T031–T034 and the WP04 origin.
...
Required tests in the new `MidiFileWriterTests.cs` (writer level) and `DacMidiExporterTests.cs`:
...
```bash
cd /home/jose/MDPlayer && dotnet test tests/MDPlayer.Fmp.Tests/ --filter "FullyQualifiedName~MidiFileWriterTests|FullyQualifiedName~DacMidiExporterTests"
```
