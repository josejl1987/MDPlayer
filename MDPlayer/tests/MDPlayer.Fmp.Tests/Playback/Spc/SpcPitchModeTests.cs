...
/// <summary>
/// SPC pitch-mode behavioral coverage (restored after PR4). Proves:
///  - the CLI --spc-pitch flag flows into the canonical request (default:
///    estimate);
///  - relative mode skips root estimation while estimate mode runs it;
///  - the mode is plumbed end-to-end through SpcPlaybackBackend.Open.
/// </summary>
public sealed class SpcPitchModeTests
{
    // ---- CLI parsing: --spc-pitch flows through the generic renderer command ----

    [Fact]
    public void Cli_ParsesSpcPitchRelative()
    {
        var result = RenderCommandParser.ParseCore(
            ["track.spc", "--spc-pitch", "relative"]);
        Assert.Equal(SpcPitchInterpretation.Relative, result.Request.Playback.SpcPitch);
    }
    ...
    {
        var result = RenderCommandParser.ParseCore(
            ["track.spc", "--spc-pitch=estimate"]);
        Assert.Equal(SpcPitchInterpretation.Estimate, result.Request.Playback.SpcPitch);
    }

    [Fact]
    public void Cli_SpcPitchDefaultsToEstimate()
    {
        var result = RenderCommandParser.ParseCore(["track.spc"]);
        Assert.Equal(SpcPitchInterpretation.Estimate, result.Request.Playback.SpcPitch);
    }

    [Fact]
    public void Cli_ToLayoutSettings_PreservesSpcPitch()
    {
        // The resolved layout settings flow downstream from the request; the
        // pitch interpretation must survive the projection untouched.
        var result = RenderCommandParser.ParseCore(
            ["track.spc", "--spc-pitch", "relative"]);
        Assert.Equal(SpcPitchInterpretation.Relative, result.Request.Playback.SpcPitch);
    }
    ...
    public void RelativeMode_InstrumentBuild_SkipsRootEstimation()
    {
        var builder = new SpcInstrumentBuilder(
            pitchAccuracy: "relative", enablePitchEstimation: false);
    ...
    {
        var builder = new SpcInstrumentBuilder(
            pitchAccuracy: "relative", enablePitchEstimation: true);
        SpcInstrumentDefinition def = builder.Resolve(BuildSnapshot(), 0);

        Assert.True(def.EstimatedRootHz.HasValue,
            $"expected an estimated root, got null (hash {def.SampleHash})");
        Assert.InRange(def.RootConfidence, 0, 1);
        Assert.True(def.PitchAccuracy is "estimated" or "pitched",
            $"unexpected pitch accuracy {def.PitchAccuracy}");
    }
    ...
    public void Backend_RelativeMode_AllInstrumentsHaveNoEstimatedRoot()
    {
        ...
        using SpcPlaybackSession session = (SpcPlaybackSession)new SpcPlaybackBackend().Open(
            ...
            new PlaybackOptions(FadeSeconds: 0, MaxDurationSeconds: 0.5,
                SpcPitchMode: SpcPitchMode.Relative),
            new Sink());
    ...
    public void Backend_EstimateMode_AtLeastOneInstrumentHasEstimatedRoot()
    {
        ...
        Assert.NotEmpty(session.Instruments);
        Assert.Contains(session.Instruments,
            def => def.EstimatedRootHz.HasValue);
    }
    ...
    /// <summary>
    /// Full in-memory .spc file (valid signature, CPU hangs at 0x0200) whose
    /// voice 0 references the 400 Hz looping sine at RAM 0x0400.
    /// </summary>
    ...
    {
        // The native library is auto-discovered under the test bin's
        // runtimes/linux-x64/native layout. Mirror that resolution here so the
        // mode-plumbing tests set the environment variable explicitly and still
        // find the built library.
        string[][] roots =
        {
            new[] { AppContext.BaseDirectory, "runtimes", "linux-x64", "native" },
            new[] { AppContext.BaseDirectory, "native" },
        };
        string name = OperatingSystem.IsWindows() ? "mdplayer_spc.dll" : "libmdplayer_spc.so";
        foreach (string[] segments in roots)
        {
            string candidate = Path.Combine(segments.Concat(new[] { name }).ToArray());
        ...
        {
            Path.Combine(root, "native", "MDPlayer.SpcNative", "build", name),
        };
        return repoCandidates.FirstOrDefault(c => File.Exists(c));
    }
