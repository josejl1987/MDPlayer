...
public sealed class VisualizationV3ContractTests
{
    ...
    [SkippableFact]
    public void OpenClSemanticRendererProducesDeterministicGpuFrames()
    {
        VisualizationGpuProbe probe = VisualizationGpuSupport.Probe();
        Skip.If(!probe.Supported, probe.Reason);

        VisualizationTimeline timeline = VisualizationTimelineFixture.Create();
        using var gpu = new PanelOverlayRenderer(
            timeline,
            new PanelOverlayRenderer.Options
            {
                Width = 960,
        ...
        byte[] gpuFrame = new byte[gpu.FrameByteCount];
        byte[] gpuAgain = new byte[gpu.FrameByteCount];
        byte[] gpuSequential = new byte[gpu.FrameByteCount];
        byte[] cpuFrame = new byte[cpu.FrameByteCount];
        gpu.RenderCompositeFrame(30, ReadOnlySpan<byte>.Empty, gpuFrame);
        gpu.RenderCompositeFrame(30, ReadOnlySpan<byte>.Empty, gpuAgain);
        SequentialCompositeSession session = gpu.CreateSequentialSession();
        session.Initialize(gpuSequential);
        session.RenderNext(30, ReadOnlySpan<byte>.Empty, gpuSequential);
        cpu.RenderCompositeFrame(30, ReadOnlySpan<byte>.Empty, cpuFrame);
        byte[] gpuHash = SHA256.HashData(gpuFrame);
        byte[] cpuHash = SHA256.HashData(cpuFrame);

        gpu.RenderCompositeFrame(31, ReadOnlySpan<byte>.Empty, gpuFrame);
        byte[][] parallelFrames = [new byte[gpu.FrameByteCount], new byte[gpu.FrameByteCount]];
        Parallel.For(0, parallelFrames.Length, index =>
            gpu.RenderCompositeFrame(30, ReadOnlySpan<byte>.Empty, parallelFrames[index]));

        Assert.Equal(gpuHash, SHA256.HashData(gpuAgain));
        Assert.Equal(gpuHash, SHA256.HashData(gpuSequential));
        Assert.Equal(gpuHash, SHA256.HashData(parallelFrames[0]));
        Assert.Equal(gpuHash, SHA256.HashData(parallelFrames[1]));
        Assert.NotEqual(cpuHash, gpuHash);
        Assert.Contains(gpuFrame, value => value != 0);

        for (int frame = 0; frame < 4; frame++)
            gpu.RenderCompositeFrame(frame, ReadOnlySpan<byte>.Empty, gpuFrame);
        GC.Collect();
