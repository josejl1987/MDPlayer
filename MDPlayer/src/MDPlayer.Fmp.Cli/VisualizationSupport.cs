        ...
        EncoderFallbackState encoderFallback = null)
    {
        ...
        Console.WriteLine(JsonSerializer.Serialize(new
        {
            ...
            stages = new
            {
                ...
                sourcePlaybackStateSeconds = captureSeconds,
                stemExportSeconds = stemRenderSeconds,
                audioProcessingSeconds = stemRenderSeconds,
                ...
                muxFinalizationSeconds = composeMetrics?.MuxFinalizationSeconds ?? 0,
                ...
                scopeFrameReadSeconds = composeMetrics?.ScopeFrameReadSeconds ?? 0,
                renderSeconds = composeMetrics?.Renderer?.RenderSeconds ?? 0,
                dynamicLayerSeconds = composeMetrics?.Renderer?.DynamicSeconds ?? 0,
                frameStateUpdateSeconds = composeMetrics?.Renderer?.FrameStateSeconds ?? 0,
                compositingSeconds = composeMetrics?.Renderer?.CompositingSeconds ?? 0,
                layoutSeconds = composeMetrics?.Renderer?.LayoutSeconds ?? 0,
                staticLayerSeconds = composeMetrics?.Renderer?.StaticLayerSeconds ?? 0,
                textSeconds = composeMetrics?.Renderer?.TextSeconds ?? 0,
                pianoRollSeconds = composeMetrics?.Renderer?.PianoRollSeconds ?? 0,
                waveformSeconds = composeMetrics?.Renderer?.WaveformSeconds ?? 0,
                fullRedraws = composeMetrics?.Renderer?.FullRedraws ?? 0,
                partialRedraws = composeMetrics?.Renderer?.PartialRedraws ?? 0,
                unchangedFrames = composeMetrics?.Renderer?.UnchangedFrames ?? 0,
                renderedPixels = composeMetrics?.Renderer?.RenderedPixels ?? 0,
                avoidedPixels = composeMetrics?.Renderer?.AvoidedPixels ?? 0,
                surfaceCopies = composeMetrics?.Renderer?.SurfaceCopies ?? 0,
                fullFrameCopies = composeMetrics?.Renderer?.FullFrameCopies ?? 0,
                scopeCopies = composeMetrics?.Renderer?.ScopeCopies ?? 0,
                copiedBytes = composeMetrics?.Renderer?.CopiedBytes ?? 0,
                sourceCursorAdvances = composeMetrics?.Renderer?.SourceCursorAdvances ?? 0,
                pianoRollCursorAdvances = composeMetrics?.Renderer?.PianoRollCursorAdvances ?? 0,
                visibleNotesVisited = composeMetrics?.Renderer?.VisibleNotesVisited ?? 0,
                allocatedBytes = composeMetrics?.Renderer?.AllocatedBytes ?? 0,
                allocatedBytesPerFrame = composeMetrics?.Renderer?.AllocatedBytesPerFrame ?? 0,
                peakWorkingSetBytes = composeMetrics?.Renderer?.PeakWorkingSetBytes ?? 0,
                queueWaitSeconds = composeMetrics?.QueueWaitSeconds ?? 0,
                rendererIdleSeconds = composeMetrics?.QueueWaitSeconds ?? 0,
