...
/// <summary>
/// YM2203's three-channel OPN FM portion. The integrated SSG is kept as an
/// explicit future capability; FM notes remain fully synchronized today.
/// </summary>
internal sealed class Ym2203TimelineDecoder : IChipTimelineDecoder
{
    ...
    public ChipType ChipType => ChipType.Ym2203;
    ...
    public void Initialize(DeviceDescriptor device, TimelineBuilder timeline)
    {
        ...
        foreach (VoiceDescriptor voice in VisualizationDeviceCatalog.Ym2203Voices(device.Id.Instance))
    ...
    public void Process(in TimedChipWrite write)
    {
        ...
        if (write.Port == 0 && address == 0x28)
        {
            int channel = write.Data & 0x03;
            if (channel < 3)
            {
                ...
                else
                {
                    ...
                    string id = $"ym2203:{_device.Id.Instance}:fm:{channel}";
        ...
        if (write.Port == 0 && (address is <= 5 or 7 or >= 8 and <= 10))
        {
            for (int channel = 0; channel < 3; channel++)
                ReconcileSsg(channel, write.SamplePosition);
            ReconcileSsgNoise(write.SamplePosition);
            return;
        }

        int pitchChannel = address switch
        {
            >= 0xA0 and <= 0xA2 => address - 0xA0,
            >= 0xA4 and <= 0xA6 => address - 0xA4,
            _ => -1,
        };
        if (write.Port == 0 && pitchChannel >= 0)
            _notes[pitchChannel]?.AddPitch(write.SamplePosition, DecodePitch(pitchChannel));
    }
    ...
    {
        int fNumber = _registers[0xA0 + channel]
            | ((_registers[0xA4 + channel] & 0x07) << 8);
        int block = (_registers[0xA4 + channel] >> 3) & 0x07;
        if (fNumber <= 0)
    ...
    private void ReconcileSsg(int channel, long sample)
    {
        ...
        {
            string id = $"ym2203:{_device.Id.Instance}:ssg:{channel}";
            _timeline.AddInstrument(new InstrumentDefinition(
    ...
    private void ReconcileSsgNoise(long sample)
    {
        if ((_registers[7] & 0x38) == 0x38)
            return;
        int volume = _registers[8..11].Select(value => value & 0x0F).DefaultIfEmpty().Max();
        if (volume > 0)
    ...
    private void CloseSsg(int channel, long endSample)
    {
